...
export default function ClientDetail() {
  ...
  const balance = getClientPaymentSummary(client)
  const paidPct = balance.total > 0 ? Math.min(Math.round((balance.paid / balance.total) * 100), 100) : 0
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Cabecera: identidad, estado, acciones y saldo a un toque */}
      <div className="space-y-4 rounded-2xl border bg-card p-4 shadow-sm sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex min-w-0 items-center gap-4">
            <Link to="/clientes" className="shrink-0">
              <Button variant="outline" size="icon" className="rounded-xl" aria-label="Volver a clientes">
                <ArrowLeft className="h-4 w-4" />
              </Button>
            </Link>
            <div className="grid h-[52px] w-[52px] shrink-0 place-items-center rounded-2xl bg-secondary text-lg font-extrabold text-secondary-foreground">
              {initials}
            </div>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2.5">
                <h1 className="truncate text-2xl font-extrabold tracking-tight">{fullName}</h1>
                <StatusSelect value={client.status} onChange={handleStatusChange} />
              </div>
              <p className="mt-1 text-sm text-muted-foreground">
                {[client.llc_name, client.state].filter(Boolean).join(' · ')}
                {primaryPhone && (
                  <>
                    {client.llc_name || client.state ? ' · ' : ''}
                    <span className="font-mono">{primaryPhone}</span>
                  </>
                )}
              </p>
              {!client.first_name && !client.last_name && (
                <p className="text-sm text-muted-foreground">Sin nombre — agrega datos desde Editar</p>
              )}
            </div>
          </div>

          {/* Acciones primarias — a un toque */}
          <div className="flex flex-wrap items-center gap-2">
            ...
            {whatsappNumber && (
              <Button
                size="sm"
                variant="outline"
                className="h-10 rounded-xl text-green-700 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300"
                onClick={() => {
                  window.open(`https://wa.me/${whatsappNumber}`, '_blank')
                  handleContactClick('WhatsApp')
                }}
              >
                <svg viewBox="0 0 24 24" className="mr-1 h-4 w-4 fill-current" aria-hidden="true">
        ...
        {/* Barra de saldo agregada */}
        {balance.hasAny && balance.total > 0 && (
      ...
      <div className="lg:grid lg:grid-cols-5 lg:gap-6 space-y-6 lg:space-y-0">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <ClientTimeline client={client} calls={calls} />
